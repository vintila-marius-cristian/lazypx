...
End of Update():
  ...
  detail.Sync(state)             ← detail reads state.Selected
  fires loadVMExtrasCmd()        ← asynchronous background Cmd to get Config & Guest IPs
View() called on next frame → detail pane shows UI, soon re-renders when VMExtrasLoadedMsg finishes
```
...
Update() → handleKey() → returns tea.Cmd (goroutine)
  ...
  → state.AddTask(upid, node, label) → watchChannelCmd()
      stream: taskLogStreamMsg{} → state.AppendTaskLog() + returns recursive tea.Cmd to keep reading
      done:   taskDoneStreamMsg{} → state.MarkTaskDone() → cache.Invalidate() → refreshCluster()
```
